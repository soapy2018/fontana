...
public class DataFilterProperties {
     ...
     * 是否启用租户过滤。
     */
    @Value("${fontana.datafilter.tenant.enabled:false}")
    private Boolean enabledTenantFilter;

    /**
     * 是否启动数据权限过滤。
     */
    @Value("${fontana.datafilter.dataperm.enabled:false}")
     ...
     * 用于拼接数据权限过滤的SQL语句。
     */
    @Value("${fontana.datafilter.dataperm.deptRelationTablePrefix:}")
     ...
     * 加上原有表名时，SQL语法会报错。
     */
    @Value("${fontana.datafilter.dataperm.addTableNamePrefix:true}")
