...
public class SqlKeyword {
	...
	/**
	 * 条件构造器
	 * 例子：前端传入 map对象<buyAmount_gt,100>，解析成 qw.gt(buy_amount, 100);
	 * @param query 查询字段
	 * @param qw    查询包装类
	 */
	public static void buildCondition(Map<String, Object> query, QueryWrapper<?> qw) {
		if (SupUtil.isEmpty(query)) {
			return;
		}
		query.forEach((k, v) -> {
			if (SupUtil.hasEmpty(k, v) || k.endsWith(IGNORE)) {
				return;
			}
			if (k.endsWith(EQUAL)) {
	...
	private static String getColumn(String column, String keyword) {
		return StringUtil.camelToUnderline(StringUtil.removeEndIgnoreCase(column, keyword));
	}
