...
import com.bluetron.nb.common.base.upload.UploadStoreInfo;
...
public class MyModelUtil {
    ...
    public static String safeMapToColumnName(String fieldName, Class<?> modelClazz) {
        ...
        if (columnName == null) {
            log.error("获取对象{}-字段{}的数据库列名称失败", modelClazz.getSimpleName(), fieldName);
            throw new GeneralException("获取对象" + modelClazz.getSimpleName() + "字段" + fieldName +"的数据库列名称失败");
        }
        return columnName;
    }
    ...
    public static <M> void fillCommonsForInsert(M data) {
        try {
            ...
            log.error("初始化通用字段数据出错：{}", e);
            throw new GeneralException("初始化通用字段数据出错");
        }
    }
