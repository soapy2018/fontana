...
public class MyWhereCriteria {
     ...
     *
     * @exception GeneralException selectFieldList中存在非法实体字段时，抛出该异常。
     * @return 组装后的SQL条件从句。
     */
     ...
     * @exception GeneralException selectFieldList中存在非法实体字段时，抛出该异常。
     * @exception GeneralException 参数modelClazz没有对应的table，抛出该异常。
     * @return 组装后的SQL条件从句。
     */
    public String makeCriteriaString(Class<?> modelClazz) {
        ...
        if (modelClazz != null) {
            ...
            if (fieldInfo == null) {
                throw new GeneralException("对象：" + modelClazz.getSimpleName() + "字段：" + fieldName + "转化成数据表字段异常");
            }
            columnName = fieldInfo.getFirst();
            columnType = fieldInfo.getSecond();
            tableName = MyModelUtil.mapToTableName(modelClazz);
            if (tableName == null) {
                throw new GeneralException("非法实体：" + modelClazz.getSimpleName());
            }
        } else {
