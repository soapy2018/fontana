...
import com.bluetron.nb.common.db.util.DataAutorUtils;
...
import com.bluetron.nb.common.util.tools.JwtUtil;
import com.bluetron.nb.common.util.tools.SqlInjectionUtil;
...
public class QueryGenerator {
	...
	private static void installMplus(QueryWrapper<?> queryWrapper,Object searchObj,Map<String, String[]> parameterMap) {
		...
		*/
		...
		Map<String, SysPermissionDataRuleVo> ruleMap = getRuleMap();
	...
	private static void doMultiFieldsOrder(QueryWrapper<?> queryWrapper,Map<String, String[]> parameterMap) {
		...
		if (StringUtil.isNotEmpty(column) && StringUtil.isNotEmpty(order)) {
			//字典字段，去掉字典翻译文本后缀
			if(column.endsWith(CommonConstants.DICT_TEXT_SUFFIX)) {
				column = column.substring(0, column.lastIndexOf(CommonConstants.DICT_TEXT_SUFFIX));
			}
			//SQL注入check
			...
			if (order.toUpperCase().indexOf(ORDER_TYPE_ASC)>=0) {
				...
				String columnStr = StringUtil.camelToUnderline(column);
                    ...
                    for (int i = 0; i < conditions.size(); i++) {
                        ...
                        if (StringUtil.isNotEmpty(rule.getField())
                            ...
                            if("date".equals(rule.getType())){
								queryValue = DateUtil.stringToDateNoTime(rule.getVal());
							}else if("datetime".equals(rule.getType())){
								queryValue = DateUtil.stringToDate(rule.getVal());
							}
	 ...
	 * @return
	 */
	public static Map<String, SysPermissionDataRuleVo> getRuleMap() {
		Map<String, SysPermissionDataRuleVo> ruleMap = new HashMap<String, SysPermissionDataRuleVo>();
		List<SysPermissionDataRuleVo> list = DataAutorUtils.loadDataSearchConditon();
		if(list != null&&list.size()>0){
			if(list.get(0)==null){
				return ruleMap;
			}
			for (SysPermissionDataRuleVo rule : list) {
	...
	private static void addRuleToQueryWrapper(SysPermissionDataRuleVo dataRule, String name, Class propertyType, QueryWrapper<?> queryWrapper) {
		...
		if(rule.equals(QueryRuleEnum.IN) && ! propertyType.equals(String.class)) {
			...
			if (propertyType.equals(String.class)) {
				...
				if(dateStr.length()==10){
					addEasyQuery(queryWrapper, name, rule, DateUtil.stringToDateNoTime(dateStr));
				}else{
					addEasyQuery(queryWrapper, name, rule, DateUtil.stringToDate(dateStr));
				}
			}else {
	 ...
	 * @return
	 */
	private static String getSingleSqlByRule(QueryRuleEnum rule,String field,Object value,boolean isString, DbType dataBaseType) {
		String res = "";
		switch (rule) {
		case GT:
	 ...
	 * @return
	 */
	private static String getFieldConditionValue(Object value,boolean isString, DbType dataBaseType) {
		...
		if(dataBaseType==null){
			dataBaseType = JdbcUtil.getDatabaseTypeEnum();
		}
		if(isString) {
			if(DbType.SQL_SERVER.equals(dataBaseType)){
				return " N'"+str+"' ";
			}else{
				return " '"+str+"' ";
			}
			...
			if(DbType.SQL_SERVER.equals(dataBaseType) && str.endsWith("'") && str.startsWith("'")){
				return " N"+str;
			}
	...
	private static String getInConditionValue(Object value,boolean isString) {
		...
		if(isString) {
			...
			for (String string : temp) {
				if(DbType.SQL_SERVER.equals(JdbcUtil.getDatabaseTypeEnum())){
					res.add("N'"+string+"'");
				}else{
					res.add("'"+string+"'");
				}
	...
	public static String installAuthJdbc(Class<?> clazz) {
		...
		for (int i = 0; i < origDescriptors.length; i++) {
			...
			if(ruleMap.containsKey(name)) {
				...
				SysPermissionDataRuleVo dataRule = ruleMap.get(name);
				QueryRuleEnum rule = QueryRuleEnum.getByValue(dataRule.getRuleConditions());
				...
				String filedSql = getSingleSqlByRule(rule, StringUtil.camelToUnderline(column), value,isString);
	...
	public static String getAllConfigAuth() {
		...
		String sql_and = " and ";
		for (String c : ruleMap.keySet()) {
			SysPermissionDataRuleVo dataRule = ruleMap.get(c);
	...
	private static String specialStrConvert(String value) {
		if (DbType.MYSQL.equals(JdbcUtil.getDatabaseTypeEnum()) || DbType.MARIADB.equals(JdbcUtil.getDatabaseTypeEnum())) {
