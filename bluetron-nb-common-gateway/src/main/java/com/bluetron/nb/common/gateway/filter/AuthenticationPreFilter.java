...
public class AuthenticationPreFilter implements GlobalFilter, Ordered {
    ...
    public Mono<Void> filter(ServerWebExchange exchange, GatewayFilterChain chain) {
        ...
        if (JwtUtil.isNullOrExpired(c)) {
            ...
            byte[] responseBody = JSON.toJSONString(Result.failed(ResultCode.USER_NOT_LOGGED_IN,
        ...
        try {
            ...
            log.error("Failed to call AuthenticationPreFilter.com.bluetron.nb.common.gateway.filter.", e);
        }
        ...
        if (Boolean.FALSE.equals(isAdmin) && !this.hasPermission(redissonClient, sessionId, url)) {
            ...
            response.getHeaders().setContentType(MediaType.APPLICATION_JSON);
            byte[] responseBody = JSON.toJSONString(Result.failed(ResultCode.NO_ACCESS_PERMISSION,
