...
import com.bluetron.nb.common.gateway.config.ApplicationConfig;
...
public class AuthenticationPostFilter implements GlobalFilter, Ordered {
                ...
                return super.writeWith(fluxBody.buffer().map(dataBuffers -> {
                    ...
                    if (StringUtils.isBlank(responseBody)) {
                        ...
                        responseBody = JSON.toJSONString(
                                Result.failed(ResultCode.INTERFACE_INNER_INVOKE_ERROR, errorMessage));
                    ...
                    try {
                        ...
                        result = JSON.toJSONString(
                                Result.failed(ResultCode.INTERFACE_INNER_INVOKE_ERROR));
                    }
    ...
    @Override
    public int getOrder() {
        // -1 is response write com.bluetron.nb.common.gateway.filter, must be called before that
        return -2;
    }
    ...
    private String doProcess(ServerWebExchange exchange, String responseBody) {
        // 这个解析出来的就是upms登录或登出接口返回的Result对象。
        ServerHttpRequest originalRequest = exchange.getRequest();
        if (originalRequest.getURI().getPath().equals(GatewayConstant.ADMIN_LOGIN_URL)) {
            // 处理登录服务的消息体，同时重构该消息体，并最终返回前端。
            Result<JSONObject> result = processLoginResponse(responseBody);
            return JSON.toJSONString(result);
        }
        if (originalRequest.getURI().getPath().equals(GatewayConstant.ADMIN_LOGOUT_URL)) {
            Result<Void> result = JSON.parseObject(responseBody, Result.class);
            if (result.isSuccess()) {
    ...
    private Result<JSONObject> processLoginResponse(String responseBody) {
        ...
        if (tokenData == null) {
            return Result.failed(ResultCode.INTERFACE_INNER_INVOKE_ERROR, "内部错误，用户登录令牌对象没有正确返回！");
        }
        Long userId = tokenData.getLong("userId");
        if (ObjectUtil.isBlankOrNull(userId)) {
            return Result.failed(ResultCode.INTERFACE_INNER_INVOKE_ERROR, "内部错误，用户Id没有正确返回！");
        }
        Boolean isAdmin = tokenData.getBoolean("isAdmin");
        if (isAdmin == null) {
            return Result.failed(ResultCode.INTERFACE_INNER_INVOKE_ERROR, "内部错误，是否为管理员标记没有正确返回！");
        }
        String showName = tokenData.getString("showName");
        if (StringUtils.isBlank(showName)) {
            return Result.failed(ResultCode.INTERFACE_INNER_INVOKE_ERROR, "内部错误，用户显示名没有正确返回！");
        }
        String loginName = tokenData.getString("loginName");
        if (StringUtils.isBlank(loginName)) {
            return Result.failed(ResultCode.INTERFACE_INNER_INVOKE_ERROR, "内部错误，用户登录名没有正确返回！");
        }
        String sessionId = tokenData.getString("sessionId");
        if (StringUtils.isBlank(sessionId)) {
            return Result.failed(ResultCode.INTERFACE_INNER_INVOKE_ERROR, "内部错误，SESSION_ID没有正确返回！");
        }
