...
import com.fontana.util.tools.AopTargetUtil;
...
public abstract class AbstractBaseService<M, K extends Serializable> extends ServiceImpl<BaseDaoMapper<M>, M> implements IBaseService<M, K> {
    ...
    /**
     * 当前Service关联的主Model对象的所有本地服务系统字典关联的结构列表，该字段在系统启动阶段一次性预加载，提升运行时效率。
     */
    private final List<LocalRelationStruct> localRelationSysDictStructList = new LinkedList<>();
    /**
    ...
    /**
     * 当前Service关联的主Model对象的所有远程系统字典关联的结构列表，该字段在系统启动阶段一次性预加载，提升运行时效率。
     */
    private final List<RemoteRelationStruct> remoteRelationSysDictStructList = new LinkedList<>();
    /**
     ...
     *
     * @return true忽略，否则抛出GeneralException异常。
     */
        ...
        if (CollUtil.isNotEmpty(dataList)) {
            ...
            for (M updateData : updateDataList) {
                ...
                if (updateUserIdFieldName != null && ReflectUtil.getFieldValue(updateData, updateUserIdFieldName) == null) {
                    ReflectUtil.setFieldValue(updateData, updateUserIdFieldName, WebContextUtil.getUserId());
                }
    ...
    /**
     * 返回符合 filter.k = filter.v 条件的一条数据。
     *
     * @param filter 过滤的Java字段: 过滤的Java字段值。
     * @return 查询后的数据对象。
     */
    public M getOne(JSONObject filter) {
        QueryWrapper<M> queryWrapper = new QueryWrapper<>();
        filter.forEach((f, v)->{
            String columnName = this.safeMapToColumnName(f);
            queryWrapper.eq(columnName, v);
        });
        return mapper().selectOne(queryWrapper);
    }

//    public <T> List<M> getDictItemList(String dictCodeName, String dictCode, String itemValueName, Set<T> itemValues) {
//        if (CollectionUtils.isEmpty(itemValues)) {
//            return new LinkedList<>();
//        }
//        QueryWrapper<M> queryWrapper = new QueryWrapper<M>();
//        queryWrapper.eq(this.safeMapToColumnName(dictCodeName), dictCode)
//                .in(this.safeMapToColumnName(itemValueName), itemValues);
//        return mapper().selectList(queryWrapper);
//    }

    /**
     * 根据指定where条件返回查询结果。(基本是内部框架使用，不建议外部接口直接使用)。
     *
     * @param criteriaList  where查询条件。
     * @return 查询结果。
     */
    public List<M> getInList(List<MyWhereCriteria> criteriaList) {
        String whereClause = MyWhereCriteria.makeCriteriaString(criteriaList, modelClass);
        return this.getListByCondition(null, null, whereClause, null);
    }
     ...
     * @return 查询结果对象。
     * @throws GeneralException ignoreRpcError()方法返回false，同时远程服务调用出现错误时抛出此异常。
     */
    @Override
    ...
    public  <R> CallResult verifyImportForConstDict(List<M> dataList, String fieldName, Function<M, R> idGetter) {
        ...
        if (field == null) {
            throw new GeneralException("FieldName [" + fieldName + "] doesn't exist.");
        }
        RelationConstDict relationConstDict = field.getAnnotation(RelationConstDict.class);
        if (relationConstDict == null) {
            throw new GeneralException("FieldName [" + fieldName + "] doesn't have RelationConstDict.");
        }
    ...
    public <R> CallResult verifyImportForDict(List<M> dataList, String fieldName, Function<M, R> idGetter) {
        ...
        if (relationDict == null) {
            throw new GeneralException("FieldName [" + fieldName + "] doesn't have RelationDict.");
        }
        if (StringUtils.isNotBlank(relationDict.slaveServiceName())) {
            AbstractBaseService<Object, Serializable> service = SpringContextHolder.getBean(
            ...
            BaseClient<Object, Object, Object> client = (BaseClient<Object, Object, Object>)
                    SpringContextHolder.getBean(relationDict.slaveClientClass());
            Result<Pagination<Object>> result = client.listBy(new MyQueryParam());
            if (!result.isSuccess()) {
                String errorMessage = "数据验证失败，字段 [" + fieldName + "] 获取关联数据失败！";
                return CallResult.error(errorMessage);
            }
            Set<Object> dictIdSet = result.getData().getDataList().stream()
                    .map(c -> ReflectUtil.getFieldValue(c, relationDict.slaveIdField())).collect(toSet());
            for (M data : dataList) {
                R id = idGetter.apply(data);
                if (id != null && !dictIdSet.contains(id)) {
    ...
    public <R> CallResult verifyImportForDatasourceDict(List<M> dataList, String fieldName, Function<M, R> idGetter) {
        ...
        if (StringUtils.isNotBlank(relationDict.slaveServiceName())) {
            AbstractBaseService<Object, Serializable> slaveService = SpringContextHolder.getBean(
                    StringUtils.uncapitalize(relationDict.slaveServiceName()));
            ...
            Result<List<?>> result = client.notExist(queryParam);
            if (!result.isSuccess()) {
                String errorMessage = "数据验证失败，字段 [" + fieldName + "] 获取关联数据失败！";
                return CallResult.error(errorMessage);
            }
            List<?> notExistIdList = result.getData();
            if (CollUtil.isNotEmpty(notExistIdList)) {
    ...
    public <R> CallResult verifyImportForOneToOneRelation(List<M> dataList, String fieldName, Function<M, R> idGetter) {
        ...
        if (relationOneToOne == null) {
            throw new GeneralException("FieldName [" + fieldName + "] doesn't have RelationOneToOne.");
        }
        ...
        if (StringUtils.isNotBlank(relationOneToOne.slaveServiceName())) {
            ...
            @SuppressWarnings("unchecked")
            BaseClient<Object, Object, Object> client = (BaseClient<Object, Object, Object>)
                    SpringContextHolder.getBean(relationOneToOne.slaveClientClass());
            MyQueryParam queryParam = new MyQueryParam();
    ...
    public void buildRelationForDataList(
        ...
        boolean dataFilterValue = DataFilterThreadLocal.setDataFilter(false);
        try {
            // 集成本地一对一和字段级别的数据关联。
            ...
            if (relationParam.isBuildDict()) {
                ...
                // 构建本地系统字典
                this.buildSysDictForDataList(resultList, buildOneToOne, ignoreFields);
            }
            // 集成远程一对一和字段级别的数据关联。
            ...
            if (relationParam.isBuildRemoteDict()) {
                ...
                //远程系统字典
                this.buildRemoteSysDictForDataList(resultList, buildRemoteOneToOne, ignoreFields);
            }
            // 组装本地聚合计算关联数据
            ...
            DataFilterThreadLocal.setDataFilter(dataFilterValue);
        }
    }
    ...
    public <T extends M> void buildRelationForData(T dataObject, MyRelationParam relationParam, Set<String> ignoreFields) {
        ...
        try {
            ...
            if (relationParam.isBuildDict()) {
                ...
                // 构建系统字典关联关系。
                this.buildSysDictForData(dataObject, buildOneToOne, ignoreFields);
            }
            boolean buildRemoteOneToOne =
                    relationParam.isBuildRemoteOneToOne() || relationParam.isBuildRemoteOneToOneWithDict();
            if (buildRemoteOneToOne) {
            ...
            if (relationParam.isBuildRemoteDict()) {
                ...
                this.buildRemoteSysDictForData(dataObject, buildRemoteOneToOne, ignoreFields);
            }
            // 组装本地聚合计算关联数据
            if (relationParam.isBuildAggregation()) {
                // 构建一对多场景
    ...
    private void buildRemoteOneToOneForDataList(List<M> resultList, boolean withDict, Set<String> ignoreFields) {
        ...
        for (RemoteRelationStruct relationStruct : this.remoteRelationOneToOneStructList) {
            ...
            Result<Pagination<Object>> result = relationStruct.remoteClient.listBy(queryParam);
            if (result.isSuccess()) {
                ...
                this.logErrorOrThrowException(result.getMsg());
            }
        }
    }
    ...
    private <T extends M> void buildRemoteOneToOneForData(T dataObject, boolean withDict, Set<String> ignoreFields) {
        ...
        for (RemoteRelationStruct relationStruct : this.remoteRelationOneToOneStructList) {
            ...
            if (id != null) {
                ...
                queryParam.addCriteriaList(whereCriteria);
                Result<Object> result = relationStruct.remoteClient.getBy(queryParam);
                if (result.isSuccess()) {
     ...
     * @throws GeneralException ignoreRpcError()方法返回false，同时远程服务调用出现错误时抛出此异常。
     */
    private void buildRemoteSysDictForDataList(List<M> resultList, boolean hasBuiltOneToOne, Set<String> ignoreFields) {
        if (CollectionUtils.isEmpty(this.remoteRelationSysDictStructList) || CollectionUtils.isEmpty(resultList)) {
            return;
        }
        for (RemoteRelationStruct relationStruct : this.remoteRelationSysDictStructList) {
            if (ignoreFields != null && ignoreFields.contains(relationStruct.relationField.getName())) {
                continue;
            }
            ...
            if (hasBuiltOneToOne && relationStruct.equalOneToOneRelationField != null) {
                ...
                queryParam.setUseDataFilter(false);
                MyWhereCriteria whereCriteria1 = new MyWhereCriteria();
                //dict_code = relationSysDict.dictCode()
                whereCriteria1.setCriteria(
                        relationStruct.relationSysDict.dictCodeName(), MyWhereCriteria.OPERATOR_EQUAL, relationStruct.relationSysDict.dictCode());
                queryParam.addCriteriaList(whereCriteria1);
                MyWhereCriteria whereCriteria2 = new MyWhereCriteria();
                //item_value in masterIdSet
                whereCriteria2.setCriteria(
                        relationStruct.relationSysDict.itemValueName(), MyWhereCriteria.OPERATOR_IN, masterIdSet);
                queryParam.addCriteriaList(whereCriteria2);
                Result<Pagination<Object>> result = relationStruct.remoteClient.listBy(queryParam);
                // 成功或者没有数据
                if (result.isSuccess()) {
                    relationList = result.getData().getDataList();
                } else {
                    logErrorOrThrowException(result.getMsg());
                }
            }
            MyModelUtil.makeSysDictRelation(
                    modelClass, resultList, relationList, relationStruct.relationField.getName());
        }
    }
     ...
     * @throws GeneralException ignoreRpcError()方法返回false，同时远程服务调用出现错误时抛出此异常。
     */
    private <T extends M> void buildRemoteSysDictForData(T dataObject, boolean hasBuiltOneToOne, Set<String> ignoreFields) {
        if (dataObject == null || CollectionUtils.isEmpty(this.remoteRelationSysDictStructList)) {
            return;
        }
        for (RemoteRelationStruct relationStruct : this.remoteRelationSysDictStructList) {
            if (ignoreFields != null && ignoreFields.contains(relationStruct.relationField.getName())) {
            ...
            if (hasBuiltOneToOne && relationStruct.equalOneToOneRelationField != null) {
                ...
                queryParam.addCriteriaList(whereCriteria1);
                MyWhereCriteria whereCriteria2 = new MyWhereCriteria();
                //item_value  = masterIdSet
                whereCriteria2.setCriteria(
                        relationStruct.relationSysDict.itemValueName(), MyWhereCriteria.OPERATOR_EQUAL, id);
                queryParam.addCriteriaList(whereCriteria2);

                Result<Object> result = relationStruct.remoteClient.getBy(queryParam);
                if (result.isSuccess()) {
//                    relationObject = this.normalizeData(
//                            result.getData(), relationStruct.relationSysDict.slaveModelClass());
                      relationObject = result.getData();
                } else {
                    this.logErrorOrThrowException(result.getMsg());
                }
            }
    ...
    private void buildOneToOneForDataList(List<M> resultList, boolean withDict, Set<String> ignoreFields) {
        ...
        for (LocalRelationStruct relationStruct : this.localRelationOneToOneStructList) {
            ...
            AbstractBaseService<Object, Serializable> relationService = relationStruct.localService;
            ...
            if (withDict && relationStruct.relationOneToOne.loadSlaveDict()
                ...
                //系统字典
                proxyTarget.buildSysDictForDataList(relationList, false, ignoreFields);
                // 关联远程字典。
                //业务字典
                proxyTarget.buildRemoteDictForDataList(relationList, false, ignoreFields);
                //系统字典
                proxyTarget.buildRemoteSysDictForDataList(relationList, false, ignoreFields);
            }
        }
    }
        ...
        for (LocalRelationStruct relationStruct : this.localRelationOneToOneStructList) {
            ...
            if (id != null) {
                ...
                if (withDict && relationStruct.relationOneToOne.loadSlaveDict() && relationObject != null) {
                    ...
                    //系统字典
                    proxyTarget.buildSysDictForData(relationObject, false, ignoreFields);
                    // 关联远程字典。
                    proxyTarget.buildRemoteDictForData(relationObject, false, ignoreFields);
                    proxyTarget.buildRemoteSysDictForData(relationObject, false, ignoreFields);
                }
            }
        }
    }
     ...
     * @param ignoreFields     该集合中的字段，即便包含注解也不会在当前调用中进行数据组装。
     */
    private void buildSysDictForDataList(List<M> resultList, boolean hasBuiltOneToOne, Set<String> ignoreFields) {
        if (CollectionUtils.isEmpty(this.localRelationSysDictStructList) || CollectionUtils.isEmpty(resultList)) {
            return;
        }
        for (LocalRelationStruct relationStruct : this.localRelationSysDictStructList) {
            if (ignoreFields != null && ignoreFields.contains(relationStruct.relationField.getName())) {
                continue;
            }
            List<Object> relationList = null;
            if (hasBuiltOneToOne && relationStruct.equalOneToOneRelationField != null) {
                ...
                Set<Object> masterIdValueSet = resultList.stream()
                        .map(obj -> ReflectUtil.getFieldValue(obj, relationStruct.masterIdField))
                        .filter(Objects::nonNull)
                        .collect(toSet());
                if (CollectionUtils.isNotEmpty(masterIdValueSet)) {

                    MyQueryParam queryParam = new MyQueryParam(false);
                    queryParam.setUseDataFilter(false);
                    List<MyWhereCriteria> myWhereCriteriaList = new ArrayList<>(2);
                    MyWhereCriteria whereCriteria1 = new MyWhereCriteria();
                    //dict_code = relationSysDict.dictCode()
                    whereCriteria1.setCriteria(
                            relationStruct.relationSysDict.dictCodeName(), MyWhereCriteria.OPERATOR_EQUAL, relationStruct.relationSysDict.dictCode());
                    myWhereCriteriaList.add(whereCriteria1);
                    MyWhereCriteria whereCriteria2 = new MyWhereCriteria();
                    //item_value  = masterIdSet
                    whereCriteria2.setCriteria(
                            relationStruct.relationSysDict.itemValueName(), MyWhereCriteria.OPERATOR_IN, masterIdValueSet);
                    myWhereCriteriaList.add(whereCriteria2);

                    relationList = relationStruct.localService.getInList(myWhereCriteriaList);

//                    relationList = relationStruct.localService.getDictItemList(
//                            relationStruct.relationSysDict.dictCodeName(),
//                            relationStruct.relationSysDict.dictCode(),
//                            relationStruct.relationSysDict.itemValueName(),
//                            masterIdValueSet);
                }
            }
            MyModelUtil.makeSysDictRelation(
     ...
     * @param ignoreFields     该集合中的字段，即便包含注解也不会在当前调用中进行数据组装。
     */
    private <T extends M> void buildSysDictForData(T dataObject, boolean hasBuiltOneToOne, Set<String> ignoreFields) {
        if (dataObject == null || CollectionUtils.isEmpty(this.localRelationSysDictStructList)) {
            return;
        }
        for (LocalRelationStruct relationStruct : this.localRelationSysDictStructList) {
            if (ignoreFields != null && ignoreFields.contains(relationStruct.relationField.getName())) {
            ...
            if (hasBuiltOneToOne && relationStruct.equalOneToOneRelationField != null) {
                ...
                if (masterIdValue != null) {
                    ...
                    jsonObject.put(relationStruct.relationSysDict.dictCodeName(), relationStruct.relationSysDict.dictCode());
                    jsonObject.put(relationStruct.relationSysDict.itemValueName(), masterIdValue);
                    relationObject = relationStruct.localService.getOne(jsonObject);
                }
            }
            MyModelUtil.makeSysDictRelation(
                    modelClass, dataObject, relationObject, relationStruct.relationField.getName());
     ...
     * 判断远程关联数据是否包含错误信息。
     *
     * @param Result 远程服务返回结果对象。
     * @return true表示包含错误信息，否则false。
     */
    protected boolean hasErrorOfVerifyRemoteRelatedData(Result<Boolean> Result) {
        return !Result.isSuccess() || Boolean.FALSE.equals(Result.getData());
    }
    ...
    protected String safeMapToColumnName(String fieldName) {
        String columnName = MyModelUtil.mapToColumnName(fieldName, modelClass);
        if (columnName == null) {
            throw new GeneralException("对象：" + modelClass.getSimpleName() + "字段：" + fieldName + "转化成数据表字段异常");
        }
        return columnName;
    }
    ...
    protected UpdateWrapper<M> createUpdateQueryForNullValue(M o, K id) {
        ...
        try {
            ...
            log.error("Failed to call reflection code of AbstractBaseService.createUpdateQueryForNullValue.", e);
    ...
    private void initializeRemoteRelationDictStruct(Field f) {
        //系统字典
        RelationSysDict relationSysDict = f.getAnnotation(RelationSysDict.class);
        if (relationSysDict != null) {
            if (relationSysDict.remoteClientClass().equals(DummyClass.class)) {
                return;
            }
            RemoteRelationStruct relationStruct = new RemoteRelationStruct();
            relationStruct.relationField = f;
            relationStruct.masterIdField = ReflectUtil.getField(modelClass, relationSysDict.masterIdField());
            relationStruct.relationSysDict = relationSysDict;
            if (StringUtils.isNotBlank(relationSysDict.equalOneToOneRelationField())) {
                relationStruct.equalOneToOneRelationField =
                        ReflectUtil.getField(modelClass, relationSysDict.equalOneToOneRelationField());
            }
            Object client = SpringContextHolder.getBean(relationSysDict.remoteClientClass());
        ...
        if (relationDict != null) {
            ...
            Object client = SpringContextHolder.getBean(relationDict.slaveClientClass());
            relationStruct.remoteClient = (BaseClient<Object, Object, Object>) client;
            remoteRelationDictStructList.add(relationStruct);
        }
    }
    ...
    private void initializeRemoteRelationStruct(Field f) {
        ...
        if (relationOneToOne != null) {
            ...
            Object client = SpringContextHolder.getBean(relationOneToOne.slaveClientClass());
            relationStruct.remoteClient = (BaseClient<Object, Object, Object>) client;
    ...
    private void initializeRemoteRelationAggregationStruct(Field f) {
        ...
        if (relationOneToManyAggregation != null) {
            ...
            Object client = SpringContextHolder.getBean(relationOneToManyAggregation.slaveClientClass());
            relationStruct.remoteClient = (BaseClient<Object, Object, Object>) client;
        ...
        if (relationManyToManyAggregation != null) {
            ...
            Object client = SpringContextHolder.getBean(relationManyToManyAggregation.slaveClientClass());
            relationStruct.remoteClient = (BaseClient<Object, Object, Object>) client;
    ...
    private void initializeLocalRelationStruct(Field f) {
        ...
        if (relationOneToOne != null) {
            ...
            relationStruct.relationOneToOne = relationOneToOne;
            if (StringUtils.isNotBlank(relationOneToOne.slaveServiceName())) {
                relationStruct.localService = SpringContextHolder.getBean(
                        StringUtils.uncapitalize(relationOneToOne.slaveServiceName()));
            } else {
                relationStruct.localService = (AbstractBaseService<Object, Serializable>)
        ...
        if (relationManyToMany != null) {
            ...
            relationStruct.manyToManyMapper = SpringContextHolder.getBean(
    ...
    private void initializeLocalRelationDictStruct(Field f) {
        ...
        if (relationSysDict != null) {
            ...
            if (StringUtils.isNotBlank(relationSysDict.dictServiceName())) {
                relationStruct.localService = SpringContextHolder.getBean(
                        StringUtils.uncapitalize(relationSysDict.dictServiceName()));
            }
            localRelationSysDictStructList.add(relationStruct);
            return; //一个字段只能有一个字典类注解
        }
        //关联常量类字典
        ...
        if (relationDict != null) {
            ...
            if (StringUtils.isNotBlank(relationDict.slaveServiceName())) {
                relationStruct.localService =
                        SpringContextHolder.getBean(StringUtils.uncapitalize(relationDict.slaveServiceName()));
            } else {
                relationStruct.localService = (AbstractBaseService<Object, Serializable>)
    ...
    private void logErrorOrThrowException(String errorMsg) {
        log.error("RPC Failed with Error message {}", errorMsg);
    ...
    private void processRemoteManyToManyAggregationWithRelationModel(
        ...
        if (CollectionUtils.isNotEmpty(mapList)) {
            ...
            queryParam.setUseDataFilter(false);
            Result<Pagination<Map<String, Object>>> result = relationStruct.remoteClient.listMapBy(queryParam);
            if (!result.isSuccess()) {
                this.logErrorOrThrowException(result.getMsg());
                return;
            }
    ...
    static class LocalRelationStruct extends RelationStruct {
        ...
        private AbstractBaseService<Object, Serializable> localService;
        ...
        private RelationSysDict relationSysDict;
