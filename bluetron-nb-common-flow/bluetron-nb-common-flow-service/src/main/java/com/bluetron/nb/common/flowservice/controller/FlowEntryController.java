...
import com.bluetron.nb.common.flowapi.dict.FlowTaskType;
import com.bluetron.nb.common.flowapi.dto.FlowEntryDto;
import com.bluetron.nb.common.flowapi.vo.FlowEntryPublishVo;
import com.bluetron.nb.common.flowapi.vo.FlowEntryVo;
import com.bluetron.nb.common.flowapi.vo.TaskInfoVo;
import com.bluetron.nb.common.flowservice.entity.FlowEntry;
import com.bluetron.nb.common.flowservice.entity.FlowEntryPublish;
import com.bluetron.nb.common.flowservice.entity.FlowEntryVariable;
...
import com.bluetron.nb.common.flowservice.service.FlowEntryVariableService;
...
@Slf4j
@RestController
@RequestMapping("${bluetron-nb-common.common-flow.urlPrefix}/flowEntry")
public class FlowEntryController {
     ...
     */
    @PostMapping("/add")
    public Result<Long> add(@MyRequestBody FlowEntryDto flowEntryDto) {
        String errorMessage = ValidateUtil.getModelValidationError(flowEntryDto);
        ...
        return Result.succeed(flowEntry.getEntryId());
    }
     ...
     */
    @PostMapping("/update")
    public Result<Void> update(@MyRequestBody FlowEntryDto flowEntryDto) {
        String errorMessage = ValidateUtil.getModelValidationError(flowEntryDto, Default.class, UpdateGroup.class);
        if (errorMessage != null) {
            return Result.failed(ResultCode.DATA_VALIDATED_FAILED, errorMessage);
        }
        FlowEntry flowEntry = MyModelUtil.copyTo(flowEntryDto, FlowEntry.class);
     ...
     */
    @PostMapping("/delete")
    public Result<Void> delete(@MyRequestBody Long entryId) {
        String errorMessage;
        if (ObjectUtil.isBlankOrNull(entryId)) {
     ...
     */
    @PostMapping("/publish")
    public Result<Void> publish(@MyRequestBody(required = true) Long entryId) throws XMLStreamException {
        String errorMessage;
        ...
        Result<TaskInfoVo> taskInfoResult = this.verifyAndGetInitialTaskInfo(flowEntry);
     ...
     */
    @PostMapping("/list")
    public Result<Pagination<FlowEntryVo>> list(
        ...
        return Result.succeed(MyPageUtil.makeResponseData(flowEntryList, FlowEntry.INSTANCE));
    }
     ...
     */
    @GetMapping("/view")
    public Result<FlowEntryVo> view(@RequestParam Long entryId) {
        if (ObjectUtil.isBlankOrNull(entryId)) {
            return Result.failed(ResultCode.PARAM_IS_BLANK);
        }
        ...
        return Result.succeed(flowEntryVo);
    }
    ...
    public Result<List<FlowEntryPublishVo>> listFlowEntryPublish(@RequestParam Long entryId) {
        List<FlowEntryPublish> flowEntryPublishList = flowEntryService.getFlowEntryPublishList(entryId);
        return Result.succeed(MyModelUtil.copyCollectionTo(flowEntryPublishList, FlowEntryPublishVo.class));
    }
     ...
     */
    @GetMapping("/listDict")
    public Result<List<Map<String, Object>>> listDict(FlowEntry filter) {
     ...
     */
    @GetMapping("/viewDict")
    public Result<Map<String, Object>> viewDict(@RequestParam Long entryId) {
        FlowEntry flowEntry = flowEntryService.getById(entryId);
        if (flowEntry == null) {
            return Result.failed(ResultCode.DATA_NOT_EXIST);
        }
    ...
    public Result<Void> updateMainVersion(
    ...
    public Result<Void> suspendFlowEntryPublish(@MyRequestBody(required = true) Long entryPublishId) {
        String errorMessage;
    ...
    public Result<Void> activateFlowEntryPublish(@MyRequestBody(required = true) Long entryPublishId) {
        String errorMessage;
        FlowEntryPublish flowEntryPublish = flowEntryService.getFlowEntryPublishById(entryPublishId);
        if (flowEntryPublish == null) {
            errorMessage = "数据验证失败，当前流程发布版本并不存在，请刷新后重试！";
    ...
    private Result<TaskInfoVo> verifyAndGetInitialTaskInfo(FlowEntry flowEntry) throws XMLStreamException {
        String errorMessage;
        BpmnModel bpmnModel = this.convertToBpmnModel(flowEntry.getBpmnXml());
