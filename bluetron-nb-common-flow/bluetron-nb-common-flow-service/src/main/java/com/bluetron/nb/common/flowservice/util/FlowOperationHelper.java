package com.bluetron.nb.common.flowservice.util;
...
import com.bluetron.nb.common.flowapi.dict.FlowApprovalType;
import com.bluetron.nb.common.flowapi.dict.FlowEntryStatus;
import com.bluetron.nb.common.flowapi.dict.FlowTaskStatus;
import com.bluetron.nb.common.flowapi.dto.FlowTaskCommentDto;
import com.bluetron.nb.common.flowapi.dto.FlowWorkOrderDto;
...
public class FlowOperationHelper {
     ...
     * @return 流程对象。
     */
    public Result<FlowEntry> verifyAndGetFlowEntry(String processDefinitionKey) {
        String errorMessage;
        ...
        return Result.succeed(flowEntry);
    }
     ...
     * @return 返回流程对象。
     */
    public Result<FlowEntry> verifyFullAndGetFlowEntry(String processDefinitionKey) {
        String errorMessage;
        // 验证流程管理数据状态的合法性。
        Result<FlowEntry> flowEntryResult = this.verifyAndGetFlowEntry(processDefinitionKey);
     ...
     * @return 验证后的流程任务对象。
     */
        public Result<Task> verifySubmitAndGetTask(
        ...
        Result<TaskInfoVo> taskInfoResult = this.verifyAndGetRuntimeTaskInfo(task);
        if (!taskInfoResult.isSuccess()) {
            return Result.failed(taskInfoResult);
        }
     ...
     */
    public Result<HistoricProcessInstance> verifyAndHistoricProcessInstance(String processInstanceId, String taskId) {
        String errorMessage;
        // 验证流程实例的合法性。
     ...
     * @return 任务信息对象。
     */
    public Result<TaskInfoVo> verifyAndGetRuntimeTaskInfo(Task task) {
        String errorMessage;
        if (task == null) {
     ...
     * @return 第一个可执行的任务信息。
     */
    public Result<TaskInfoVo> verifyAndGetInitialTaskInfo(
     ...
     * @return 验证结果。
     */
    public Result<Void> verifyUploadOrDownloadPermission(String processInstanceId, String taskId) {
        String errorMessage;
        if (StrUtil.isBlank(taskId)) {
    ...
    public FlowWorkOrder makeWorkOrderFilter(FlowWorkOrderDto filterDto, String processDefinitionKey) {
        ...
        filter.setCreateUserId(WebContextUtil.takeTokenFromRequest().getUserId());
        return filter;
    }
