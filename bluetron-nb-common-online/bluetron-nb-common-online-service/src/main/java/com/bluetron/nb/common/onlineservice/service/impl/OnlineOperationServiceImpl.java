package com.bluetron.nb.common.onlineservice.service.impl;
...
import com.bluetron.nb.common.datafilter.config.DataFilterProperties;
import com.bluetron.nb.common.db.annotation.MyDataSourceResolver;
import com.bluetron.nb.common.base.constant.AggregationType;
import com.bluetron.nb.common.db.constant.GlobalDeletedFlag;
import com.bluetron.nb.common.base.constant.DataPermRuleType;
import com.bluetron.nb.common.db.object.MyWhereCriteria;
import com.bluetron.nb.common.base.object.Tuple2;
import com.bluetron.nb.common.onlineapi.constant.OnlineConstant;
import com.bluetron.nb.common.onlineapi.dict.*;
import com.bluetron.nb.common.onlineapi.dto.OnlineFilterDto;
import com.bluetron.nb.common.onlineservice.entity.*;
import com.bluetron.nb.common.onlineservice.mapper.OnlineOperationMapper;
import com.bluetron.nb.common.onlineservice.object.ColumnData;
import com.bluetron.nb.common.onlineservice.object.JoinTableInfo;
import com.bluetron.nb.common.onlineservice.service.OnlineDictService;
import com.bluetron.nb.common.onlineservice.service.OnlineOperationService;
import com.bluetron.nb.common.onlineservice.service.OnlineVirtualColumnService;
import com.bluetron.nb.common.onlineservice.util.OnlineDataSourceResolver;
import com.bluetron.nb.common.onlineservice.util.OnlineOperationHelper;
...
public class OnlineOperationServiceImpl implements OnlineOperationService {
    ...
    private void doBuildDataListWithDict(
        ...
        for (OnlineDict dict : dictList) {
            ...
            for (String columnName : columnNameList) {
                ...
                //todo 这里一些字典表（比如bn_sys_user）如果在其他数据库里怎么办
        ...
        if (columnData.getColumn().getPrimaryKey()) {
            if (columnData.getColumnValue() == null) {
                if ("Long".equals(columnData.getColumn().getObjectFieldType())) {
                    columnData.setColumnValue(IdUtil.nextLongId());
                } else {
                    columnData.setColumnValue(IdUtil.nextStringId());
                }
            }
            ...
            switch (columnData.getColumn().getFieldKind()) {
                ...
                case FieldKind.UPDATE_USER_ID:
                    columnData.setColumnValue(WebContextUtil.takeTokenFromRequest().getUserId());
                    break;
    ...
    private String processDataPerm(String tableName, String deptFilterColumnName, String userFilterColumnName) {
        ...
        if (StrUtil.isBlank(dataPermData)) {
            throw new GeneralException("No Related DataPerm found.");
        }
