...
import com.bluetron.nb.common.onlineapi.dto.OnlineFormDto;
...
@Slf4j
@RestController
@RequestMapping("${bluetron-nb-common.common-online.urlPrefix}/onlineForm")
public class OnlineFormController {
     ...
     */
    @PostMapping("/add")
    public Result<Long> add(@MyRequestBody OnlineFormDto onlineFormDto) {
        String errorMessage = ValidateUtil.getModelValidationError(onlineFormDto);
        ...
        return Result.succeed(onlineForm.getFormId());
    }
     ...
     */
    @PostMapping("/update")
    public Result<Void> update(@MyRequestBody OnlineFormDto onlineFormDto) {
        String errorMessage = ValidateUtil.getModelValidationError(onlineFormDto, Default.class, UpdateGroup.class);
        if (errorMessage != null) {
            return Result.failed(ResultCode.DATA_VALIDATED_FAILED, errorMessage);
        }
        OnlineForm onlineForm = MyModelUtil.copyTo(onlineFormDto, OnlineForm.class);
     ...
     */
    @PostMapping("/delete")
    public Result<Void> delete(@MyRequestBody Long formId) {
        String errorMessage;
        if (ObjectUtil.isBlankOrNull(formId)) {
     ...
     */
    @PostMapping("/list")
    public Result<Pagination<OnlineFormVo>> list(
        ...
        return Result.succeed(MyPageUtil.makeResponseData(onlineFormList, OnlineForm.INSTANCE));
    }
     ...
     */
    @GetMapping("/view")
    public Result<OnlineFormVo> view(@RequestParam Long formId) {
        if (ObjectUtil.isBlankOrNull(formId)) {
            return Result.failed(ResultCode.PARAM_IS_BLANK);
        }
        ...
        return Result.succeed(onlineFormVo);
    }
     ...
     */
    @GetMapping("/render")
    public Result<JSONObject> render(@RequestParam Long formId) {
        if (ObjectUtil.isBlankOrNull(formId)) {
            return Result.failed(ResultCode.PARAM_IS_BLANK);
        }
        OnlineForm onlineForm = onlineFormService.getByIdWithRelation(formId, MyRelationParam.full());
